...
{
    if((inst >> 7) & 0x1F)
        reg[(inst >> 7) & 0x1F] = inst & 0b11111111111111111111000000000000;
}
function auipc() //#
{
    if((inst >> 7) & 0x1F)
        reg[(inst >> 7) & 0x1F] = (inst & 0b11111111111111111111000000000000) + pc; //pc
}
function jal() //#
{
    let bf = 0;
...
function sll() //#
{
    reg[(inst >> 7) & 0x1F] = reg[((inst >> 15) & 0x1F)] << (reg[(inst >> 20) & 0x1F] & 0b11111);
}
function slt() //#
{
	i32[0] = reg[((inst >> 15) & 0x1F)];
